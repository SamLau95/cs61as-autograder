...
def xqueue_login():
    """
        Logs into the xqueue. Returns the request object.
    """
    session = requests.session()
    xqueue_login_url = urlparse.urljoin(config.XQUEUE_INTERFACE['url'], project_urls.XqueueURLs.log_in)
    success, xqueue_msg = login(session,
                                xqueue_login_url,
                                config.XQUEUE_INTERFACE['django_auth']['username'],
                                config.XQUEUE_INTERFACE['django_auth']['password'])
    if not success: print "Couldn't log in!"
    return session
...
def login(session, url, username, password):
    ...
    response = session.post(url, {'username': username, 'password': password } )
...
def parse_xreply(xreply):
    """
        ...
        Returns a success flag and the content as a dict
    """
...
def parse_xobject(xobject, queue_name):
    """
        ...
        Returns success flag and content as a dict
        """
    try:
...
def post_results_to_xqueue(session, header, body):
    ...
    (success, msg) = _http_post(session, config.XQUEUE_INTERFACE['url'] + project_urls.XqueueURLs.put_result,
                                request,
